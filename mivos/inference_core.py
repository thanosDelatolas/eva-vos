...
class InferenceCore:
    ...
    def __init__(self, prop_net:PropagationNetwork, fuse_net:FusionNet, images, num_objects, 
                    mem_profile=0, mem_freq=5, device='cuda'):
        ...
        if mem_profile == 0:
            ...
            self.k_buf_size = 105
            self.i_buf_size = 105
        elif mem_profile == 2: # modified 
            self.data_dev = device
            self.result_dev = device
            self.k_buf_size = 3
            self.i_buf_size = -1
        elif mem_profile == 3: # modified 
            self.data_dev = 'cpu'
            self.result_dev = 'cpu'
            self.q_buf_size = 3
            self.i_buf_size = 3
    ...
    def do_pass(self, key_k, key_v, idx, forward=True):
        """
    ...
    def interact(self, mask, idx, scribble=False):
        """
        ...
        if scribble:
            ...
            key_v = self.prop_net.encode_value(self.get_image_buffered(idx), qf16, mask)
        ...
        self.do_pass(key_k, key_v, idx, False)
